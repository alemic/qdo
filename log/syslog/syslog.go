...
func (w *Writer) Debug(msg string) {
	w.writeAndRetry(LOG_DEBUG, msg)
}

func (w *Writer) Debugf(format string, v ...interface{}) {
	w.writeAndRetry(LOG_DEBUG, fmt.Sprintf(format, v...))
}

func (w *Writer) Info(msg string) {
	w.writeAndRetry(LOG_INFO, msg)
}

func (w *Writer) Infof(format string, v ...interface{}) {
	w.writeAndRetry(LOG_INFO, fmt.Sprintf(format, v...))
}

func (w *Writer) Warn(msg string) {
	w.writeAndRetry(LOG_WARNING, msg)
}

func (w *Writer) Warnf(format string, v ...interface{}) {
	w.writeAndRetry(LOG_WARNING, fmt.Sprintf(format, v...))
}

func (w *Writer) Error(meta string, e error) {
	w.writeAndRetry(LOG_ERR, meta+" (err: "+e.Error()+")")
}
func (w *Writer) Errorf(format string, v ...interface{}) {
	w.writeAndRetry(LOG_ERR, fmt.Sprintf(format, v...))
}
